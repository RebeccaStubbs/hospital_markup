...
Type objective_function<Type>::operator() () {
  ...
  DATA_IVECTOR(provider_i);     // Provider indicator
  ...
  DATA_INTEGER( n_provider );      // Number of counties
  ...
  // Counties deviating from 
  for( int i=0; i<n_county; i++){
    jnll -= dnorm( county_re(i), Type(0.0), exp(log_sd_county_re), true ); // Now, adding in the county-level random effect: We want each county to be a deviation from a state-level mean
  }
  
  //On facilities
  for( int i=0; i<n_provider; i++){
    jnll -= dnorm( prov_re(i), Type(0.0), exp(log_sd_prov_re), true ); // Now, adding in the county-level random effect: We want each county to be a deviation from a state-level mean
  }
  ...
  for(size_t i = 0; i < n_i; i++){
    linpred += state_re[state_i[i]]+county_re[county_i[i]]+time_re[time_i[i]]; //To add in age as a RE, you would add: +age_re[age_i[i]]
  }
